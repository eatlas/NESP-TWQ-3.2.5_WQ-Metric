...
fs = list.files(path='eReefs/data/processed',pattern='data_.*.RData',full.names=TRUE)
foreach(z=fs) %dopar% {
    load(z)
    print(z)
    ...
    WQ_tryCatch(
    {
        ...
        save(data.idx,file=paste0('eReefs/data/indexed/data.idx_',meas, '__',region,'___',waterbody,'.RData'))
        ...
        for (y in wy) {
            ...
            save(data.idx.yr, file=paste0('eReefs/data/indexed/data.idx.',y,'_',meas, '__',region,'___',waterbody,'.RData'))
        }
        ...
        data.idx = data %>% mutate(GL=GL.seasonal) %>% mutate(Binary=WQ_WQI_binary(.),
                                                              ...
                                                              fsMAMP=WQ_WQI_mamp(.),
                                                              fsMAMP4=WQ_WQI_mamp(.,fold=4))
        save(data.idx,file=paste0('eReefs/data/indexed/data.idx.seasonal_',meas, '__',region,'___',waterbody,'.RData'))
        
        wy=unique(data.idx$waterYear)
        for (y in wy) {
            data.idx.yr = data.idx %>% dplyr:::filter(waterYear==y)
            save(data.idx.yr, file=paste0('eReefs/data/indexed/data.idx.seasonal.',y,'_',meas, '__',region,'___',waterbody,'.RData'))
        }
        ...
        save(data.idx,file=paste0('eReefs/data/indexed/data.idx.exceed_',meas, '__',region,'___',waterbody,'.RData'))
        ...
        save(data.idx,file=paste0('eReefs/data/indexed/data.idx.exceed.seasonal_',meas, '__',region,'___',waterbody,'.RData'))

    },paste0('indexeReefs.log'), msg=paste0('Generate index on raw data (Measure=',meas,', Region=',region,', Water Body=',waterbody,')'), return=TRUE)
