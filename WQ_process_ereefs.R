...
flist <- list.files(path = "eReefs/data/raw", pattern = "^ereefs.*")
#flist
...
foreach(m=meas) %do% {
    foreach(z=zones) %dopar% {
        ...
        for (i in fs.str) {
            print(i)
            load(file=paste0('eReefs/data/raw/',flist[i]))
        ...
        save(data, file=paste0('eReefs/data/processed/data_',m,'__',region,'___',waterbody,'.RData'))
    }
