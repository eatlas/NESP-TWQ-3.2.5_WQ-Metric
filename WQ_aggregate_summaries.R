...
if (nchar(add.str)>0) {
    vars=strsplit(add.str,'no')[[1]][-1]
    vars = gsub('nap', 'TSS', vars)
    add.str.label = paste0(' (excluding ',
                           paste(vars, collapse=' and '),
                           ')', collapse='')
}
if (nchar(ec.str)>0) ec.str.label = '(excluding Enclosed Coastal)'
...
unlink('meta.file_aggregations.csv')
meta.file = 
    data.frame(Filename=NA,
               ...
               Longitude=NA,
               PhotoGallery=NA)
write.table(meta.file[0,], file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=TRUE)
...
fullpath = '/export/project/WQmetric/'

foreach(src=c('niskin/','flntu/','','eReefs/','eReefs926/')) %dopar% {
    print(paste('Top foreach = ',src))
    foreach(idx=(1:nrow(index_type))[c(1,2,3,4,9,10)]) %do% {
        index=index_type$index[idx]
        print(paste('foreach idx =',index))
        logfile=paste0('aggregate_summaries',index,'.log')

        ## Map for Site/Measure
        if (SiteMeasureMap) {
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.site.year.RData'))) {
                ...
                foreach (measure= unique(data.all$Measure)) %do%  { #c('chl','nap','sd','NOx')) {
                    ...
                    WQ_tryCatch(
                    {
                        if (GraphIt) {
                            ...
                            ggsave(file=paste0('Report/Figures/spatial_map_',gsub('/','',src),'_',index,'.',temporal,'_measure.',measure,'.site_Grade_',GradeType,'.jpg'), g, width=2*nx, height=(ny*2/ratio) + 0*ratio, units='in',dpi=300)
                        ...
                        meta.file =
                            data.frame(Filename=paste0('Report/Figures/spatial_map_',gsub('/','',src),'_',index,'.',temporal,'_measure.',measure,'.site_Grade_',GradeType,'.jpg'),
                                       Title=paste0('Spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', measure, ' index scores (calculated on an ',temporal,' basis).'),
                                       Description=paste0('Spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', measure, ' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                       Attribution='Murray Logan (AIMS)',
                                       Licencing='CC-BY',
                                       Location='Great Barrier Reef',
                                       Latitude=NA,
                                       Longitude=NA,
                                       PhotoGallery='Aggregations.maps')
                        write.table(meta.file, file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=FALSE,append=TRUE)
        ...
        if (SiteSubindicatorMap) {
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.site.year',add.str,'.RData'))) {
            ...
            if (proceed) {
                ...
                yl=bbox(Polys)[2,] #range(data.all$Latitude)
                xl=bbox(Polys)[1,] #range(data.all$Longitude)
                foreach (subindicator=unique(data.all$Subindicator)) %do% {
                    ...
                    WQ_tryCatch(
                    {
                        if (GraphIt) {
                            ...
                            ggsave(file=paste0('Report/Figures/spatial_map_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.',subindicator,'.site',add.str,'_Grade_',GradeType,'.jpg'), g, width=2*nx, units='in',dpi=300)
                        ...
                        meta.file =
                            data.frame(Filename=paste0('Report/Figures/spatial_map_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.',subindicator,'.site',add.str,'_Grade_',GradeType,'.jpg'),
                                       Title=paste0('Spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', subindicator, ' ', add.str,' index scores (calculated on an ',temporal,' basis).'),
                                       Description=paste0('Spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', subindicator, ' ', add.str, ' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                       Attribution='Murray Logan (AIMS)',
                                       Licencing='CC-BY',
                                       Location='Great Barrier Reef',
                                       Latitude=NA,
        ...
        if (SiteIndicatorMap) {
            ...
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.site.year',add.str,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/spatial_map_',gsub('/','',src),'_',index,'.',temporal,'_indicator.site',add.str,'_A_Grade_',GradeType,'.jpg'), g, width=2*nx, height=(ny*2/ratio) + 0*ratio, units='in',dpi=300)
                    ...
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/spatial_map_',gsub('/','',src),'_',index,'.',temporal,'_indicator.site',add.str,'_A_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level ', add.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level ', add.str.label, ' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (ZoneMeasureWorm) {
            ...
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.zone.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.zone.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.zone.year',add.str,ec.str_temp,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_measure.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=10, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_measure.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
                                   Longitude=NA,
                                   PhotoGallery='Aggregations.worms')
                    write.table(meta.file, file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=FALSE,append=TRUE)
        ...
        if (ZoneMeasureFlatMap) {
            ...
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.zone.year',add.str,ec.str,'.RData'))) {
            ...
            if (proceed) {
                ...
                if (src=='') sz=5
                foreach (measure=unique(data.all$Measure)) %do% {
                    ...
                    WQ_tryCatch(
                    {
                        if (GraphIt) {
                            ...
                            ggsave(file=paste0('Report/Figures/simple_map_',gsub('/','',src),'_',index,'.',temporal,'_measure_',measure,'.zone',add.str,ec.str,'_A_Grade_',GradeType,'.jpg'), g, width=2*nx, height=(ny*2/ratio) + 0*ratio, units='in',dpi=300)
                        ...
                        meta.file =
                            data.frame(Filename=paste0('Report/Figures/simple_map_',gsub('/','',src),'_',index,'.',temporal,'_measure_',measure,'.zone',add.str,ec.str,'_A_Grade_',GradeType,'.jpg'),
                                       Title=paste0('Simplified (Zone mean) spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', measure, ' index scores (calculated on an ',temporal,' basis).'),
                                       Description=paste0('Simplified (Zone mean) spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', measure, ' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                       Attribution='Murray Logan (AIMS)',
                                       Licencing='CC-BY',
                                       Location='Great Barrier Reef',
                                       Latitude=NA,
                                       Longitude=NA,
                                       PhotoGallery='Aggregations.flat')
                        write.table(meta.file, file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=FALSE,append=TRUE)
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_measure.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=10, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_measure.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
                                   Longitude=NA,
                                   PhotoGallery='Aggregations.mosaic')
                    write.table(meta.file, file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=FALSE,append=TRUE)
        ...
        if (ZoneSubindicatorWorm) {
            ...
            ec.str_temp = '_with_enclosed_coastal'
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.zone.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.zone.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.zone.year',add.str,ec.str_temp,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=10, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (ZoneSubindicatorFlatMap) {
            ...
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.zone.year',add.str,ec.str,'.RData'))) {
                ...
                foreach (subindicator=unique(data.all$Subindicator)) %do% {
                    ...
                    WQ_tryCatch(
                    {
                        if (GraphIt) {
                            ...
                            ggsave(file=paste0('Report/Figures/simple_map_',gsub('/','',src),'_',index,'.',temporal,'_subindicator_',subindicator,'.zone',add.strec.str,,'_A_Grade_',GradeType,'.jpg'), g, width=2*nx, units='in',dpi=300)
                        ...
                        meta.file =
                            data.frame(Filename=paste0('Report/Figures/simple_map_',gsub('/','',src),'_',index,'.',temporal,'_subindicator_',subindicator,'.zone',add.strec.str,,'_A_Grade_',GradeType,'.jpg'),
                                       Title=paste0('Simplified (Zone mean) spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', subindicator, ' ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                       Description=paste0('Simplified (Zone mean) spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' ', subindicator, ' ', add.str.label, ' ', ec.str.label, ' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                       Attribution='Murray Logan (AIMS)',
                                       Licencing='CC-BY',
                                       Location='Great Barrier Reef',
                                       Latitude=NA,
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=(xw*1)+1, height=(xh*1)+1, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (ZoneIndicatorWorm) {
            ...
            ec.str_temp = '_with_enclosed_coastal'
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.zone.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.zone.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.zone.year',add.str,ec.str_temp,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_indicator.zone',add.str,ec.str,'_A_Grade_',GradeType,'.jpg'), g, width=10, height=10, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_indicator.zone',add.str,ec.str,'_A_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (ZoneIndicatorFlatMap) {
            ...
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.zone.year',add.str,ec.str,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_map_',gsub('/','',src),'_',index,'.',temporal,'_indicator.zone',add.str,ec.str,'_A_Grade_',GradeType,'.jpg'), g, width=2*nx, units='in',dpi=300)
                    ...
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_map_',gsub('/','',src),'_',index,'.',temporal,'_indicator.zone',add.str,ec.str,'_A_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Simplified (Zone mean) spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Simplified (Zone mean) spatio-temporal ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level ', add.str.label, ' ', ec.str.label, ' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_indicator.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=5, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_indicator.zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (WaterBodyMeasureWorm) {
            ...
            ec.str_temp = '_with_enclosed_coastal'
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.waterbody.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.waterbody.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.waterbody.year',add.str,ec.str_temp,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_measure.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=4, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_measure.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_measure.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=4*xw*1, height=xh*1, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_measure.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (WaterBodySubindicatorWorm) {
            ...
            ec.str_temp = '_with_enclosed_coastal'
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.waterbody.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.waterbody.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.waterbody.year',add.str,ec.str_temp,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=4, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g,  width=4*xw*1, height=xh*1, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        } # WaterBodySubindicatorMosaic if
        ...
        if (WaterBodyIndicatorWorm) {
            ...
            ec.str_temp = '_with_enclosed_coastal'
                        proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.waterbody.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.waterbody.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.waterbody.year',add.str,ec.str_temp,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_indicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=4, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_indicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_indicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=2, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_indicator.waterbody',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for water bodies (aggregated over management regions weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (GBRMeasureWorm) {
            print(paste0('GBRMeasureWorm '))
                        proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.gbr.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.gbr.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.measure.gbr.year',add.str,ec.str,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_measure.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=5, height=5, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_measure.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_measure.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=5, height=3, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_measure.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' measure level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
                                   Latitude=NA,
        ...
        if (GBRSubindicatorWorm) {
            print(paste0('GBRSubindicatorWorm '))
                        proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.gbr.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.gbr.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.subindicator.gbr.year',add.str,ec.str,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=5, height=5, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=(xw*1)+1, height=(xh*1)+1, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_subindicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' subindicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
        ...
        if (GBRIndicatorWorm) {
            ...
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.gbr.year',add.str,'.RData'))) {
                load(file=paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.gbr.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.gbr.year',add.str,ec.str,'.RData'))) {
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_indicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=5, height=5, units='in',dpi=300)
                    }
                    print('hereB')
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/simple_',gsub('/','',src),'_',index,'.',temporal,'_indicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatially dependent time series  of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatially dependent time series of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
        ...
        if (GBRIndicatorMosaic) {
            proceed = FALSE
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.gbr.year',add.str,'.RData'))) {
                load(file=paste0(fullpath,src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.gbr.year',add.str,'.RData'))
                proceed=TRUE
            }
            if (file.exists(paste0(src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.gbr.year',add.str,ec.str,'.RData'))) {
                load(file=paste0(fullpath,src,'data/aggregated/',index,'/',temporal,'/data.all.indicator.gbr.year',add.str,'.RData'))
            ...
            if (proceed) {
                ...
                WQ_tryCatch(
                {
                    if (GraphIt) {
                        ...
                        ggsave(file=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_indicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=2, units='in',dpi=300)
                    }
                    meta.file =
                        data.frame(Filename=paste0('Report/Figures/mosaic_',gsub('/','',src),'_',index,'.',temporal,'_indicator.gbr',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                                   Title=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                                   Description=paste0('Spatio-temporal mosaic of ',ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', index, ' indicator level for entire GBR (aggregated over management regions for enclosed coastal only and weighted by area) ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Color coding based on a ',GradeType,' grade conversion.'),
                                   Attribution='Murray Logan (AIMS)',
                                   Licencing='CC-BY',
                                   Location='Great Barrier Reef',
...
foreach(index = c('Binary','fsMAMP','fsMAMP4')) %dopar% {
    ...
    if (GraphIt) {
        ...
        ggsave(file=paste0('Report/Figures/All_indicies_',index,'.',temporal,'_measure.',measure,'_zone',add.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=10, units='in',dpi=300)
    }
    meta.file =
        data.frame(Filename=paste0('Report/Figures/All_indicies_',index,'.',temporal,'_measure.',measure,'_zone',add.str,'_Grade_',GradeType,'.jpg'),
                   Title=paste0('Comparison of different sources in time series of ', index, ' ', measure, ' ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                   Description=paste0('Comparison of different sources in time series of ', index, ' ', measure, ' ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                   Attribution='Murray Logan (AIMS)',
                   Licencing='CC-BY',
                   Location='Great Barrier Reef',
                   Latitude=NA,
                   Longitude=NA,
                   PhotoGallery='Aggregations.compareSources')
    write.table(meta.file, file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=FALSE,append=TRUE)                                                
    ...
    for (measure in ms) {
        ...
        if (nrow(dat)>0) {
            if (GraphIt) {
                ...
                ggsave(file=paste0('Report/Figures/All_sources_',src,'.',temporal,'_measure.',measure,'_zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'), g, width=10, height=10, units='in',dpi=300)
            }
            meta.file =
                data.frame(Filename=paste0('Report/Figures/All_sources_',src,'.',temporal,'_measure.',measure,'_zone',add.str,ec.str,'_Grade_',GradeType,'.jpg'),
                           Title=paste0('Comparison of different indices in time series of ', ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', measure, ' ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).'),
                           Description=paste0('Comparison of different indices in time series of ', ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' ', measure, ' ', add.str.label,' ', ec.str.label,' index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
                           Attribution='Murray Logan (AIMS)',
                           Licencing='CC-BY',
                           Location='Great Barrier Reef',
                           Latitude=NA,
                           Longitude=NA,
                           PhotoGallery='Aggregations.indices')
            write.table(meta.file, file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=FALSE,append=TRUE)                                                
        }
    }
}
...
if (GraphIt) {
    ...
    ggsave(file=paste0('Report/Figures/All_excludes_index_',index,'_',gsub('/','',src),'.',temporal,'_indicator_zone_.jpg'), g, width=10, height=10, units='in',dpi=300)
    }
meta.file =
    data.frame(Filename=paste0('Report/Figures/All_excludes_index_',index,'_',gsub('/','',src),'.',temporal,'_indicator_zone_.jpg'),
               Title=paste0('Comparison of different exclusions in time series of ', index, ' ', ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' indicator level index scores (calculated on an ',temporal,' basis).'),
               Description=paste0('Comparison of different exclusions in time series of ', index, ' ', ifelse(gsub('/','',src)=='','Satellite', gsub('/','',src)), ' indicator level index scores (calculated on an ',temporal,' basis).  Faint background color banding based on a ',GradeType,' grade conversion.  The blue vertical bar spans from mid 2009 to mid 2011 and highlights a period of high rainfall.'),
               Attribution='Murray Logan (AIMS)',
               Licencing='CC-BY',
               Location='Great Barrier Reef',
               Latitude=NA,
               Longitude=NA,
               PhotoGallery='Aggregations.compareExcludes')
write.table(meta.file, file='meta.file_aggregations.csv', sep=',',row.names=FALSE, col.names=FALSE,append=TRUE)                                                
